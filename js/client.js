(function($) {
	...
	var RulesPopup = this.RulesPopup = Popup.extend({
		...
		initialize: function(data) {
			...
			buf += '<h2>Showcharms Rules</h2>';
			buf += '<br /><b>1) </b>If a staff member tells you to do something, do it. They\'re in their position for a reason, and their job is to keep the peace. Respect their decisions to enforce rules even if they are not listed below.<br /><br />People with symbols ( ~ @ % ) before their names are staff members and will remove you from chat if they deem necessary. People with a + before their names are NOT staff, but are well known members more than capable of helping you put a team together if you\'re having difficulties.<br /><br /><b>2) </b>Swearing is allowed, so either be prepared to not take offence at whatever may be said, or don\'t follow the chat. Complaining about people swearing is the easiest way to turn the air even more blue.<br /><br /><b>3) </b>We only speak English in this chatroom. This means no foreign languages, as there\'s a very good chance we\'ll have no way to understand what you\'re saying. There is absolutely no offence intended with this, but the fact is that a chatroom is analogous to a spoken conversation. If we can\'t understand each-other, there\'s no point.<br /><br /><b>4) </b>Like the forums, we ask that you do not use chatspeak. This means that if u tlk liek dis, you will get warned, and if you keep it up, you will be banned. This kind of typing is just laziness and it shows rudeness and disrespect to your fellow chatters. Not a good first impression.<br /><br /><b>5) </b>This should go without saying, but flaming, trolling, or generally being anything other than friendly will get you removed from chat very quickly. Especially if, when warned to stop, you still continue. If you win a battle, do not gloat; if you lose a battle, don\'t whine incessantly about it.';
		/*	buf += '<b>Chat</b><br /><br /><b>1.</b> Do not spam, flame, or troll. This includes advertising, asking questions with one-word answers in the lobby, and flooding the chat such as by copy/pasting lots of text in the lobby.<br /><br /><b>2.</b> Don\'t call unnecessary attention to yourself. Don\'t be obnoxious. ALL CAPS, <i><b>formatting</b></i>, and -&gt; ASCII art &lt;- are acceptable to emphasize things, but should be used sparingly, not all the time.<br /><br /><b>3.</b> No minimodding: don\'t mod if it\'s not your job. Don\'t tell people they\'ll be muted, don\'t ask for people to be muted, and don\'t talk about whether or not people should be muted ("inb4 mute", etc). This applies to bans and other punishments, too.<br /><br /><b>4.</b> We reserve the right to tell you to stop discussing moderator decisions if you become unreasonable or belligerent.<br /><br />(Note: Chat rules don\'t apply to battle rooms, but only if both players in the battle are okay with it.)<br /><br />'; */
			...
			if (warning) {
				buf += '<p class="buttonbar"><button name="close" disabled>Close</button><small class="overlay-warn"> You will be able to close this in 15 seconds</small></p>';
				setTimeout(_.bind(this.rulesTimeout, this), 15000);
			} else {
				this.type = 'semimodal';
